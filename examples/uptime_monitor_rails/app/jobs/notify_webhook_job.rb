...
class NotifyWebhookJob < ApplicationJob
  ...
  def perform(check)
    ...
    return logger.warn("NotifyWebhookJob: WEBHOOK_URL not set; skipping") if url.empty?
  ...
  def deliver(url, payload)
    ...
    case status_code
    when 200..299
      logger.info { "NotifyWebhookJob: delivered to #{url} (HTTP #{status_code})" }
    when 400..499
      raise PermanentFailure, "Webhook receiver returned #{status_code}; giving up"
    else
